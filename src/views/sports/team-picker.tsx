...
import { matchesTeamSearch } from "@/lib/sports/search-text";
import {
  clearFavouriteTeams,
  cachedLeagueTeams,
...
export function TeamPicker({
  ...
  useEffect(() => {
    let cancelled = false;
    setLoading(true);
    setByLeague(Object.fromEntries(keys.map((key) => [key, cachedLeagueTeams(key)])));
    const queue = [...keys];
    const worker = async () => {
      while (queue.length && !cancelled) {
        const key = queue.shift()!;
        try {
          const teams = await fetchLeagueTeams(key);
          if (!cancelled && teams.length)
            setByLeague((previous) => ({ ...previous, [key]: teams }));
        } catch {
          /* Other leagues can still finish. */
        }
      }
    };
    void Promise.all(Array.from({ length: Math.min(3, keys.length) }, worker)).finally(() => {
    ...
    () =>
      keys
        .map((key) => {
          ...
          const teams = needle ? all.filter((team) => matchesTeamSearch(team, needle)) : all;
          return { key, teams };
        })
        ...
        <div ref={bodyRef} className="flex flex-1 flex-col gap-6 overflow-y-auto px-5 pb-5">
          {loading && sections.length === 0 ? (
            ...
            <p className="py-16 text-center text-[13px] text-ink-subtle">
              {needle
                ? t("No teams match that search.")
                : t("No teams available for these competitions yet.")}
            </p>
          ) : (
            sections.map((section) => (
              <LeagueSection
                key={section.key}
                leagueKey={section.key}
                teams={section.teams}
                cols={cols}
                fav={fav}
              />
    ...
    <section className="flex flex-col gap-3">
      <div className="flex items-center gap-2.5">
        ...
        <h3 className="text-[13px] font-semibold text-ink">
          {def ? getLeagueLabel(def) : leagueKey}
        </h3>
        ...
        <button
          ...
          className={`ms-auto flex h-7 items-center gap-1.5 rounded-full px-2.5 text-[11.5px] font-medium transition-colors ${
            pinned
              ? "bg-raised text-ink"
              : "bg-raised/60 text-ink-subtle hover:bg-raised hover:text-ink"
          }`}
        >
      ...
      {teamCatalogIsPartial(leagueKey) && (
        <p className="text-xs text-ink-muted">{t("This source provides a partial team list.")}</p>
      )}
      <div
        className="harbor-cascade grid gap-3"
