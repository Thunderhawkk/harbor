...
export function ReaderMatchPicker({
  ...
  title: string;
  pid: string;
  connected: MangaTracker[];
  onClose: () => void;
}) {
  const t = useT();
  // Pick one manga: search the richest canonical source (AniList) when it is
  // connected, falling back to MAL for MAL-only users. The choice is the same
  // manga regardless of which tracker is connected.
  const searchTracker: MangaTracker = connected.includes("anilist") ? "anilist" : "mal";
  ...
  const dismiss = () => {
    // A dismissal (backdrop / close / Escape without a pick) records that this
    // tracker has no usable match for the title, so sync stops re-prompting.
    if (!confirmedRef.current) {
      setMangaMatchDismissed(pid, searchTracker, normalizeTitle(title));
    }
    onClose();
  };
  ...
  const load = useCallback(
    async (q: string) => {
      setBusy(true);
      try {
        setHits(await searchCandidates(searchTracker, q));
      } catch {
        setHits([]);
      } finally {
        setBusy(false);
  ...
  }, [onClose, pid, searchTracker, title]);

  const confirm = async (chosen: MangaCandidate) => {
    setLinkingId(chosen.id);
    const titleKey = normalizeTitle(title);
    confirmedRef.current = true;
    // The C menu only guides the auto-sync: it saves the per-title mapping and
    // lets the sync runner push the chapter. One pick must apply to every
    // connected tracker or a second tracker keeps its own mapping and pushes the
    // manga the user is actually reading instead of the one chosen here.
    if (searchTracker === "anilist") {
      setMangaMatch(pid, "anilist", titleKey, String(chosen.id), chosen.title);
      if (connected.includes("mal") && typeof chosen.malId === "number") {
        setMangaMatch(pid, "mal", titleKey, String(chosen.malId), chosen.title);
      }
    } else {
      setMangaMatch(pid, "mal", titleKey, String(chosen.id), chosen.title);
      if (connected.includes("anilist")) {
        try {
          const az = await aniZipByMal(Number(chosen.id));
          if (az?.mappings?.anilist_id != null) {
            setMangaMatch(pid, "anilist", titleKey, String(az.mappings.anilist_id), chosen.title);
          }
        } catch {
          // AniZip unreachable; the picked tracker is still saved above.
        }
      }
    }
    void onClose();
      ...
      <div className="animate-fade-in fixed inset-0 z-[98] grid place-items-center px-4">
        <div className="pointer-events-auto flex max-h-[80vh] w-full max-w-md flex-col overflow-hidden rounded-2xl border border-edge-soft bg-raised/95 shadow-[0_24px_60px_-16px_rgba(0,0,0,0.75)] backdrop-blur-2xl">
          ...
          <div className="harbor-scroll min-h-0 flex-1 overflow-y-auto p-2">
            {hits === null ? (
              <p className="px-3 py-8 text-center text-[13px] text-ink-muted">{t("Searching…")}</p>
            ) : hits.length === 0 ? (
              <p className="px-3 py-8 text-center text-[13px] leading-relaxed text-ink-muted">
