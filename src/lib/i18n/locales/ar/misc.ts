const misc: Record<string, string> = {
  "Hide this tab": "إخفاء علامة التبويب هذه",
  "Show this tab": "إظهار علامة التبويب هذه",
  "Nothing hidden.": "لا توجد علامات تبويب مخفية.",
  "Edit sidebar": "تعديل الشريط الجانبي",
  "Show all tabs": "إظهار كل علامات التبويب",
  "Hide categories from recommendations. Customize tabs separately in the sidebar.":
    "إخفاء فئات من التوصيات. يمكنك تخصيص علامات التبويب بشكل منفصل في الشريط الجانبي.",
  "Hides anime titles from Home, Discover, Top 10, and catalogs. Western animation stays visible. To hide the Anime tab, edit the sidebar.":
    "يخفي عناوين الأنمي من الرئيسية والاستكشاف وأفضل 10 والكتالوجات، مع إبقاء الرسوم المتحركة الغربية ظاهرة. لإخفاء علامة تبويب الأنمي، عدّل الشريط الجانبي.",
  Collections: "المجموعات",
  "Open settings": "فتح الإعدادات",
  "Collapse sidebar": "طيّ الشريط الجانبي",
  "Expand sidebar": "توسيع الشريط الجانبي",
  "Edit profile": "تعديل الملف الشخصي",
  "Done editing": "انتهاء التعديل",
  "to close": "للإغلاق",
  "Close · Esc": "إغلاق · Esc",
  "Esc or click outside to close": "اضغط Esc أو انقر خارجًا للإغلاق",
  "Press T": "اضغط T",
  "Align {dir}": "محاذاة {dir}",
  "Failed: {message}": "فشل: {message}",
  "Remove {name}": "إزالة {name}",
  "Override {name}": "تجاوز {name}",
  "Apply {language}": "تطبيق {language}",

  "Search title…": "ابحث عن عنوان…",
  "Search actors, directors…": "ابحث عن ممثلين ومخرجين…",
  "Search countries...": "ابحث عن دول...",
  "Empty — click to add filters": "فارغ، انقر لإضافة مرشّحات",
  "Add a TMDB key to browse by this filter.": "أضف مفتاح TMDB للتصفّح حسب هذا المرشّح.",
  "Add a TMDB key in settings first": "أضف مفتاح TMDB في الإعدادات أولًا",
  "Add a TMDB key above to unlock this.": "أضف مفتاح TMDB أعلاه لفتح هذا.",
  "Add an OMDb key above to unlock this.": "أضف مفتاح OMDb أعلاه لفتح هذا.",
  "Add a TMDB key in Settings to see cast, related titles, and trailers here.":
    "أضف مفتاح TMDB في الإعدادات لرؤية طاقم العمل والعناوين ذات الصلة والإعلانات هنا.",

  "Custom CSS": "CSS مخصص",
  "Live-injected into the document. Use it to retheme buttons, change spacing, recolor anything.":
    "يُحقن مباشرة في المستند. استخدمه لتغيير ألوان الأزرار والمسافات وتخصيص المظهر بالكامل.",
  "Custom JS": "JS مخصص",
  "Runs in the app's WebView. You're modding your own client. No sandbox, no safety net. Errors land in the console.":
    "يعمل داخل WebView. أنت تقوم بتعديل نسختك. لا توجد بيئة معزولة ولا حماية. الأخطاء تظهر في Console.",
  "Custom HTML overlay": "طبقة HTML مخصصة",
  "Injected into a fixed-position layer above the app (pointer-events disabled by default). Wrap in a div with pointer-events:auto to make it interactive.":
    "تُحقن في طبقة ثابتة فوق التطبيق. ضعها في div مع pointer-events:auto لجعلها تفاعلية.",
  "Custom code": "كود مخصص",
  "Power-user knob. Inject your own CSS, JS, and HTML into Harbor. Lives in your local settings; nothing leaves your machine.":
    "أداة للمحترفين. احقن أكواد CSS و JS و HTML الخاصة بك. يتم حفظها في إعداداتك المحلية ولا شيء يغادر جهازك.",
  "You're modding your own client. Custom JS has full access to your Harbor session. Only paste code you wrote or fully trust.":
    "أنت تقوم بتعديل نسختك الخاصة. الكود يمتلك وصولاً كاملاً لجلستك. لا تلصق إلا أكواداً تثق بها تماماً.",

  "Close to the system tray": "التصغير إلى علبة النظام (System tray)",
  "Closing the window tucks Harbor into the tray instead of quitting, so it reopens instantly. Right-click the tray icon for quick controls, or pick Quit to exit fully.":
    "إغلاق النافذة يُخفي Harbor في العلبة بدلاً من إنهائه تماماً، ليُفتح لاحقاً بلمح البصر. انقر بالزر الأيمن على الأيقونة للتحكم السريع، أو اختر 'إنهاء' لإغلاقه بالكامل.",
  "Always on top": "دائماً في المقدمة",
  "Keep the Harbor window above other windows.": "إبقاء نافذة Harbor فوق النوافذ الأخرى.",
  "Pause when minimized": "إيقاف مؤقت عند التصغير",
  "Stop playback when you minimize Harbor or send it to the tray.":
    "إيقاف التشغيل مؤقتاً عند تصغير Harbor أو إخفائه في العلبة.",
  "Pause when unfocused": "إيقاف مؤقت عند فقدان التركيز",
  "Stop playback whenever another window takes focus.":
    "إيقاف التشغيل مؤقتاً عندما تنتقل للعمل على نافذة أخرى.",
  "Block ads & trackers": "حظر الإعلانات والمتتبعات",
  "Watching for ad, analytics, and tracking requests. Harbor itself sends zero telemetry.":
    "مراقبة طلبات الإعلانات والتحليلات والتتبع وحظرها. Harbor نفسه لا يرسل أي بيانات تتبع.",
  "Export everything": "تصدير كل شيء",
  "Saves your whole Harbor setup to one file: theme, home layout, settings, addons, profiles, watchlist, player layouts, watch progress, and more. Your Stremio sign-in is left out on purpose.":
    "حفظ إعدادات Harbor بالكامل في ملف واحد: المظهر، واجهة الرئيسية، الإعدادات، الإضافات، الملفات الشخصية، قائمة المشاهدة، واجهة المشغل، سجل المشاهدة، والمزيد. (يتم استثناء تسجيل الدخول لـ Stremio عمداً).",
  Export: "تصدير",
  "Restore from a backup": "استعادة من نسخة احتياطية",
  "Loads a backup file and replaces your current setup with it. Perfect for a new computer. Your Stremio sign-in on this device stays as is.":
    "تحميل ملف النسخة الاحتياطية واستبدال إعداداتك الحالية به. مثالي عند الانتقال لجهاز جديد. تسجيل الدخول لـ Stremio على هذا الجهاز لن يتأثر.",
  Restore: "استعادة",
  "Restore this backup?": "استعادة هذه النسخة؟",
  "This replaces your current Harbor setup (theme, home layout, settings, addons, profiles, and more) with the {n} saved entries in this file. Your Stremio sign-in stays as is. Harbor reloads when it finishes.":
    "سيستبدل هذا إعدادات Harbor الحالية (المظهر، الرئيسية، الإعدادات، الإضافات، الملفات الشخصية...) بـ {n} من المدخلات المحفوظة في هذا الملف. لن يتأثر حساب Stremio. سيعاد تشغيل Harbor عند الانتهاء.",
  "an unknown date": "تاريخ غير معروف",
  "Saved {when} from Harbor {app}.": "تم الحفظ في {when} من Harbor {app}.",
  "Restoring...": "جاري الاستعادة...",
  "Restore and reload": "استعادة وإعادة تشغيل",

  "Hide watched titles in catalogs": "إخفاء العناوين المُشاهدة من القوائم",
  "Movies you've watched and shows you've made progress on stop appearing in the built-in Discover rows, using your Trakt history. Needs Trakt connected. Continue Watching is never touched.":
    "لن تظهر الأفلام التي شاهدتها والمسلسلات التي أحرزت تقدماً فيها في صفوف 'اكتشف' المدمجة، بناءً على سجل Trakt الخاص بك. (يتطلب ربط حساب Trakt). قسم 'متابعة المشاهدة' لن يتأثر أبداً.",

  "Horizontal view": "عرض أفقي",
  "Grid view": "عرض شبكي",
  "List view": "عرض القائمة",
  Beta: "تجريبي",
  "How is this build treating you?": "ما رأيك في هذه النسخة؟",
  "Does Harbor {version} feel better or worse than the version you had before?":
    "هل إصدار Harbor {version} أفضل أم أسوأ من الإصدار السابق؟",
  "Much worse": "أسوأ بكثير",
  Worse: "أسوأ",
  "About the same": "بدون تغيير / عادي",
  Better: "أفضل",
  "Much better": "أفضل بكثير",
  "Rate this build": "قيّم هذا الإصدار",
  "Send rating": "إرسال التقييم",
  "You rated this build {label}.": "لقد قيّمت هذا الإصدار بـ«{label}».",
  Change: "تغيير",
  "Sorry this one is not better. Tell us what went wrong and we will fix it for you.":
    "نأسف لأن هذا الإصدار ليس أفضل. أخبرنا بما حدث وسنُصلحه لك.",
  "Open a quick issue": "افتح بلاغًا سريعًا",
  "Thanks! This helps us know the betas are heading the right way.":
    "شكرًا! هذا يساعدنا على معرفة أن الإصدارات التجريبية تسير في الاتجاه الصحيح.",
  "End ep": "الحلقة الأخيرة",
  Filler: "حشو",
  "{name}'s {sub}": "{sub} الخاص بـ {name}",

  "Tune picks": "ضبط الاختيارات",
  "Tune your Top Picks": "اضبط أبرز اختياراتك",
  "Reset to default": "الإعادة إلى الافتراضي",

  "Save look": "حفظ المظهر",
  "Save this look as a template": "حفظ هذا المظهر كقالب",
  "Save as a new template": "الحفظ كقالب جديد",
  "Name your first template": "سمِّ قالبك الأول",
  "New template name": "اسم القالب الجديد",
  "Name (optional)": "الاسم (اختياري)",
  "Overwrite {name} with this look": "استبدال {name} بهذا المظهر",
  "Set as theme backdrop": "تعيين كخلفية للسمة",
  "Click to apply · Right-click to delete": "انقر للتطبيق · انقر بالزرّ الأيمن للحذف",
  "Tighter spacing": "تباعد أضيق",
  "Wider spacing": "تباعد أوسع",
  Larger: "أكبر",
  Smaller: "أصغر",

  "See others born this day": "عرض آخرين وُلدوا في هذا اليوم",
  "See others from this place": "عرض آخرين من هذا المكان",

  "No unsaved changes": "لا توجد تغييرات غير محفوظة",
  "unsaved changes": "تغييرات غير محفوظة",
  "· currently hidden": "· مخفي حاليًا",
  'Results for "{query}"': 'نتائج "{query}"',
  "Reset to original name": "الإعادة إلى الاسم الأصلي",

  "Just the next show: {title}": "المسلسل التالي فقط: {title}",
  "This show: {title}": "هذا المسلسل: {title}",
  "This and next: + {title}": "هذا والتالي: + {title}",
  " · Series": " · مسلسل",

  "Custom poster service": "خدمة ملصقات مخصّصة",
  "Fresh tomato for 60%+, splat for under.": "طماطم طازجة لنسبة 60% فأكثر، ولطخة لما دونها.",

  "Instant Play: clicking Play queues the next stream automatically.":
    "التشغيل الفوري: النقر على تشغيل يصطفّ البث التالي تلقائيًا.",
  "Manual mode: clicking Play opens the source picker here.":
    "الوضع اليدوي: النقر على تشغيل يفتح أداة اختيار المصدر هنا.",
  "Switch to Manual in settings if you'd rather pick the source yourself.":
    "بدّل إلى اليدوي في الإعدادات إن كنت تفضّل اختيار المصدر بنفسك.",

  "{start} to {end} · {dur}": "{start} إلى {end} · {dur}",
  "Until {time} · {dur}": "حتى {time} · {dur}",
  "{watched} of {total} watched ({pct}%).": "تمت مشاهدة {watched} من {total} ({pct}%).",
  "Ramadan series, drama, films, Egyptian classics, and Gulf - all in one place.":
    "مسلسلات رمضان والدراما والأفلام والكلاسيكيات المصرية والخليجي، كلها في مكان واحد.",

  Play: "تشغيل",
  Year: "السنة",
  "Play {name}": "تشغيل {name}",
  "Slide {n}": "الشريحة {n}",
  "#{position} in {label} Today": "#{position} في {label} اليوم",
  "In Cinema": "في السينما",
  Rerun: "إعادة عرض",
  "In your watchlist": "في قائمة مشاهدتك",
  "In watchlist": "في قائمة المشاهدة",
  "Watched on Trakt": "تمت مشاهدته على Trakt",
  "New episode released since you last watched": "صدرت حلقة جديدة منذ آخر مشاهدة لك",
  "Paused on Simkl": "متوقّف مؤقتًا على Simkl",
  "Remove from Continue Watching": "إزالة من متابعة المشاهدة",
  "Rotten Tomatoes audience score": "تقييم جمهور Rotten Tomatoes",
  "Next:": "التالي:",
  "No program info": "لا توجد معلومات عن البرنامج",
  "Open details": "فتح التفاصيل",
  "Unpin channel": "إلغاء تثبيت القناة",
  "Pin to top": "التثبيت في الأعلى",
  System: "النظام",
  English: "الإنجليزية",
  Version: "الإصدار",
  "Clear all": "مسح الكل",
  "Show adult addons": "إظهار إضافات البالغين",
  Stremio: "Stremio",
  Active: "نشط",
  Auto: "تلقائي",
  Hidden: "مخفي",
  "Show subtitles in Picture-in-Picture": "عرض الترجمات في صورة داخل صورة",
  Font: "الخط",
  Alignment: "المحاذاة",
  Opacity: "الشفافية",
  "Upload font": "رفع خط",
  "Delete current": "حذف الحالي",
  "Rename current": "إعادة تسمية الحالي",
  "Background opacity": "شفافية الخلفية",
  "Seek back": "رجوع",
  "Seek forward": "تقديم",
  Connected: "متصل",
  Email: "البريد الإلكتروني",
  Reveal: "إظهار",
  "Reveal comments": "إظهار التعليقات",
  "Comments are hidden": "التعليقات مخفية",
  "You haven't commented yet": "لم تعلق بعد",
  "Reveal image": "إظهار الصورة",

  "Trakt Comments": "تعليقات Trakt",
  "Open on Trakt": "فتح على Trakt",

  "AniList Comments": "تعليقات AniList",
  "Connect your AniList account to see forum threads and comments.":
    "اربط حساب AniList لعرض مواضيع المنتدى والتعليقات.",
  "Connect AniList": "ربط AniList",
  "Could not find this title on AniList.": "تعذّر العثور على هذا العمل على AniList.",
  "New thread": "موضوع جديد",
  "Back to threads": "العودة للمواضيع",
  "Thread title": "عنوان الموضوع",
  "Thread body (optional)": "نص الموضوع (اختياري)",
  "Create thread": "إنشاء موضوع",
  "Failed to create thread": "تعذّر إنشاء الموضوع",
  "Open on AniList": "فتح على AniList",
  "This thread is locked.": "هذا الموضوع مغلق.",
  "No threads for this title yet.": "لا توجد مواضيع لهذا العمل بعد.",
  "Be the first to start a discussion.": "كن أول من يبدأ النقاش.",
  "Load more threads": "تحميل المزيد من المواضيع",
  "No comments yet": "لا توجد تعليقات بعد",
  "Loading more": "جاري تحميل المزيد",
  "Failed to post comment": "تعذّر نشر التعليق",

  My: "تعليقاتي",

  Likes: "الإعجابات",
  Newest: "الأحدث",
  Oldest: "الأقدم",
  "Contains spoiler": "يحتوي على حرق",
  "Write a comment...": "اكتب تعليقًا...",
  "Comments may take a moment to appear on Trakt": "قد تستغرق التعليقات لحظة لتظهر على Trakt",
  "Spoiler — Click to reveal": "حرق، انقر للإظهار",
  "Spoiler — Click": "حرق، انقر",
  "Run test": "تشغيل الاختبار",
  "Use a different URL": "استخدام رابط مختلف",
  "Deploy a relay": "نشر المرحل (Deploy a relay)",
  Copied: "تم النسخ",
  Continue: "متابعة",
  Types: "الأنواع",
  "Click any control in the live preview to move, hide, or reorder it.":
    "انقر على أي عنصر تحكم في المعاينة الحية لنقله، إخفائه، أو إعادة ترتيبه.",
  Default: "الافتراضي",
  "Top Rated": "الأعلى تقييماً",
  "Nothing here yet": "لا يوجد شيء هنا بعد",
  "Invite via link": "دعوة عبر الرابط",
  "Watch Together needs a relay.": "المشاهدة معاً تتطلب اتصالاً وسيطاً (Relay).",
  Join: "انضمام",
  "Play tonight": "تشغيل الليلة",
  Skip: "تخطي",
  "Not interested": "غير مهتم",
  "More info": "مزيد من المعلومات",
  "Start Watching": "بدء المشاهدة",
  "Tune your recommendations": "تحسين توصياتك",
  "Thumbs down hides this title from Featured. Thumbs up helps surface similar picks.":
    "التقييم السلبي يخفي هذا العنوان من قائمة المميزة. التقييم الإيجابي يساعد في إظهار اختيارات مشابهة.",
  "Show me more like this": "عرض المزيد من هذا القبيل",
  "Show me less like this": "عرض أقل من هذا القبيل",
  "United States": "الولايات المتحدة",
  "United Kingdom": "المملكة المتحدة",
  Canada: "كندا",
  Australia: "أستراليا",
  Japan: "اليابان",
  "South Korea": "كوريا الجنوبية",
  China: "الصين",
  France: "فرنسا",
  Germany: "ألمانيا",
  Spain: "إسبانيا",
  Italy: "إيطاليا",
  India: "الهند",
  Mexico: "المكسيك",
  Brazil: "البرازيل",
  Crime: "جريمة",
  Documentary: "وثائقي",
  Family: "عائلي",
  History: "تاريخ",
  Thriller: "إثارة",
  War: "حرب",
  Western: "غربي",
  "Tune your picks": "ضبط اختياراتك",
  "More of what you love.": "المزيد مما تحب.",
  "Tap the genres you want more of. They steer the Top Picks row at the top of this page.":
    "انقر على التصنيفات التي تريد المزيد منها. فهي توجه صف الاختيارات الأفضل في أعلى هذه الصفحة.",
  "None yet": "لا شيء بعد",
  Overview: "نظرة عامة",
  "Top rated": "الأعلى تقييمًا",
  "By community stars": "حسب تقييم المجتمع",
  "Just added": "أُضيف حديثًا",
  "Freshest on stremio-addons.net": "أحدث الإضافات على stremio-addons.net",
  Installed: "تم التثبيت",
  Updated: "تم التحديث",
  "Installed locally": "تم التثبيت محلياً",
  "Install failed.": "فشل التثبيت.",
  Removed: "تمت الإزالة",
  "Couldn't remove. Try again.": "تعذرت الإزالة. حاول مرة أخرى.",
  "Hide adult addons": "إخفاء إضافات البالغين",
  Filters: "الفلاتر",
  "Browse by category": "تصفح حسب الفئة",
  "Worth knowing": "معلومات تهمك",
  "No addons installed yet": "لا توجد إضافات مثبتة بعد",
  "Configure & install": "إعداد وتثبيت",
  "Install default": "التثبيت الافتراضي",
  Reconfigure: "إعادة التهيئة",
  "stremio:// link": "رابط stremio://",
  Rate: "تقييم",
  "On Stremio-Addons": "في Stremio-Addons",
  "Project information": "معلومات المشروع",
  "Pulled from manifest": "مستخرج من ملف الـ manifest",
  "Manifest URL": "رابط الـ Manifest",
  Resources: "الموارد",
  "ID prefixes": "بادئات المعرف",
  P2P: "نظير لنظير (P2P)",
  Yes: "نعم",
  No: "لا",
  "Configure on the addon's setup page": "قم بالتهيئة في صفحة إعداد الإضافة",
  "Or paste the install link manually": "أو الصق رابط التثبيت يدويًا",
  "Hang tight, won't be a sec.": "انتظر قليلًا، لن يستغرق الأمر سوى لحظات.",
  "No velocity data yet": "لا توجد بيانات سرعة بعد",
  Install: "تثبيت",
  "Set up": "إعداد",
  "Six places to start. Tap one and we'll filter the catalog for you.":
    "ستة أماكن للبدء. انقر على إحداها وسنقوم بتصفية الدليل من أجلك.",
  "Manage addon": "إدارة الإضافة",
  "Install addon": "تثبيت إضافة",
  "Add from URL": "إضافة عبر رابط",
  Update: "تحديث",
  "Open setup page": "فتح صفحة الإعداد",
  "https://...manifest.json or stremio://...": "https://...manifest.json أو stremio://...",
  Read: "قراءة",
  Reading: "جاري القراءة",
  Updating: "جاري التحديث",
  Installing: "جاري التثبيت",
  addon: "إضافة",
  "is now using your new configuration.": "يستخدم الآن إعداداتك الجديدة.",
  "is ready. Open Discover or hit Play on a title to use it.":
    "أصبح جاهزاً. افتح 'اكتشف' أو اضغط على 'تشغيل' لأي عنوان لاستخدامه.",
  "Reading manifest": "جاري قراءة ملف manifest",
  "Reading new manifest": "جاري قراءة ملف manifest الجديد",
  "Swapping configuration": "جاري تبديل الإعدادات",
  "Couldn't read that addon URL.": "تعذرت قراءة رابط الإضافة هذا.",
  "Holdup Matey!": "توقف يا صديقي!",
  Nevermind: "تراجع",
  "Set sail": "أبحر",
  "That's not it. Try a fresh round in a moment.": "إجابة خاطئة. حاول مرة أخرى بعد قليل.",
  "Welcome aboard": "مرحباً بك على متن السفينة",
  "Recent searches": "عمليات البحث الأخيرة",
  People: "أشخاص",
  "Get started": "ابدأ الاستخدام",
  Episode: "حلقة",
  "Browse by Genre": "تصفح حسب التصنيف",
  "Choose a source to save offline. You can track progress on the Downloads page.":
    "اختر مصدراً للحفظ دون اتصال. يمكنك متابعة التقدم في صفحة التنزيلات.",
  "Last source wasn't actually cached on your debrid yet. Pick another from the list.":
    "لم يكن المصدر الأخير مخزنًا مؤقتًا بالفعل على debrid الخاص بك. اختر مصدراً آخر من القائمة.",
  "Harbor couldn't resolve a usable ID for this title. Add a TMDB key in Library settings or sign in to Stremio to broaden coverage.":
    "لم يتمكن Harbor من حل معرّف صالح لهذا العمل. أضف مفتاح TMDB في إعدادات المكتبة أو سجّل الدخول إلى Stremio لتوسيع التغطية.",
  "No playable streams turned up, and no debrid is configured. Real-Debrid, TorBox, AllDebrid, Premiumize, or Debrid-Link will unlock raw torrent results. Some addons bake debrid in (Sootio, Comet/ElfHosted, MediaFusion/ElfHosted) and play without your own keys.":
    "لم تظهر أي مصادر قابلة للتشغيل، ولم يتم تكوين debrid. سيؤدي استخدام Real-Debrid أو TorBox أو AllDebrid أو Premiumize أو Debrid-Link إلى إظهار نتائج التورنت الخام. بعض الإضافات تدمج debrid داخلها وتعمل بدون مفاتيحك الخاصة.",
  "Set up a debrid": "إعداد debrid",
  "Only 1 source after filtering": "مصدر واحد فقط بعد التصفية",
  "Only 2 sources after filtering": "مصدران فقط بعد التصفية",
  "Clean releases for this title haven't surfaced yet. The result below may not match the title you're looking for, so confirm the filename and size before playing.":
    "لم تظهر إصدارات نظيفة لهذا العمل بعد. قد لا تتطابق النتيجة أدناه مع ما تبحث عنه، لذا تأكد من اسم الملف وحجمه قبل التشغيل.",
  "Clean releases for this title are still scarce. Confirm the filename and size before playing.":
    "لا تزال الإصدارات النظيفة لهذا العمل نادرة. تأكد من اسم الملف وحجمه قبل التشغيل.",
  "Show everything anyway": "إظهار كل شيء على أي حال",
  "No source returned a stream": "لم يرجع أي مصدر نتائج",
  "Anime sources are usually richer through Torrentio's anime config or AIOStreams. Make sure one is installed in Stremio.":
    "غالبًا ما تكون مصادر الأنمي أغنى من خلال تكوين أنمي Torrentio أو AIOStreams. تأكد من تثبيت أحدهما في Stremio.",
  "On the web, Harbor can only reach addons that allow browser access (Torrentio, TorBox, Cinemeta). For unreleased titles, no source typically exists yet.":
    "على الويب، لا يمكن لـ Harbor الوصول إلا إلى الإضافات التي تسمح بوصول المتصفح. بالنسبة للأعمال غير الصادرة، لا يوجد مصدر عادةً.",
  "Try signing in to Stremio so Harbor can use your addon collection. Older or foreign titles often need Torrentio + a debrid addon to find anything.":
    "حاول تسجيل الدخول إلى Stremio حتى يتمكن Harbor من استخدام مجموعة إضافاتك. غالبًا ما تحتاج الأعمال القديمة أو الأجنبية إلى Torrentio + إضافة debrid للعثور على أي شيء.",
  "Strict filters dropped everything": "الفلاتر الصارمة تجاهلت كل شيء",
  "Search wider": "بحث أوسع",
  "Wide search · still empty": "بحث واسع · لا يزال فارغًا",
  "Not out yet": "لم يصدر بعد",
  "Filters off · still empty": "الفلاتر متوقفة · لا يزال فارغًا",
  "This source": "هذا المصدر",
  "We could not find a working stream": "لم نتمكن من العثور على مصدر يعمل",
  "· A debrid key (TorBox, Real-Debrid, etc.) is missing or expired.":
    "· مفتاح debrid (TorBox، Real-Debrid، إلخ) مفقود أو منتهي الصلاحية.",
  "· No stream addon is installed yet (Torrentio, MediaFusion, Comet).":
    "· لم يتم تثبيت أي إضافة بث حتى الآن (Torrentio، MediaFusion، Comet).",
  "· This title is too new and no source has it cached yet.":
    "· هذا العمل جديد جدًا ولم يقم أي مصدر بتخزينه مؤقتًا بعد.",
  "Browse streams manually": "تصفح المصادر يدويًا",
  "Send a bug report": "إرسال تقرير خطأ",
  "Debrid is down": "Debrid معطل",
  "No streaming sources yet": "لا توجد مصادر بث بعد",
  "· Install a stream addon (Torrentio, Comet, MediaFusion).":
    "· تثبيت إضافة بث (Torrentio، Comet، MediaFusion).",
  "· Add a debrid key (TorBox, Real-Debrid, AllDebrid, Premiumize, Debrid-Link).":
    "· إضافة مفتاح debrid (TorBox، Real-Debrid، AllDebrid، Premiumize، Debrid-Link).",
  "Browse addons": "تصفح الإضافات",
  seeders: "مزوّدون (Seeders)",
  "this title": "هذا العنوان",
  via: "عبر",
  "Streams from peers": "بث من الأقران",
  "found by": "وُجد بواسطة",
  "No debrid configured": "لا يوجد Debrid مُعد",
  "uncached on debrid": "غير مخزن مؤقتاً على Debrid",
  Background: "الخلفية",
  "Drop shadow": "ظل الإسقاط",
  "Soft halo around the text. Cleanest on most content.":
    "هالة ناعمة حول النص. الأفضل لمعظم المحتوى.",
  Outline: "إطار",
  "Hard stroke around each letter. High contrast.": "حد صلب حول كل حرف. تباين عالٍ.",
  "Black bar": "شريط أسود",
  "Rounded background panel behind the text. Most readable.":
    "لوحة خلفية دائرية خلف النص. الأكثر قابلية للقراءة.",
  "Styled (ASS) subtitles": "ترجمات مخصصة (ASS)",
  "Keep original": "الاحتفاظ بالأصل",
  "Styled (ASS) subs keep their own fonts, colors, and effects. Truest to the release.":
    "الترجمات المخصصة (ASS) تحتفظ بخطوطها وألوانها وتأثيراتها الخاصة. الأقرب للإصدار الأصلي.",
  "Resize only": "تغيير الحجم فقط",
  "Keep the original look but apply your size and position.":
    "الاحتفاظ بالمظهر الأصلي مع تطبيق الحجم والموضع الخاص بك.",
  "Use my style": "استخدام التنسيق الخاص بي",
  "Outline thickness": "سُمك الإطار",
  "Hide subtitles when the player shrinks into the floating PiP window.":
    "إخفاء الترجمات عندما يتقلص المشغل إلى نافذة PiP العائمة.",
  "Distance from bottom": "المسافة من الأسفل",
  Left: "يسار",
  Center: "وسط",
  Right: "يمين",
  "Text color": "لون النص",
  "Outline color": "لون الإطار",
  "Box color": "لون المربع",
  Inter: "انتر",
  Rounded: "مستدير",
  Serif: "مذيل",
  "Couldn't read that font file.": "تعذر قراءة ملف الخط هذا.",
  "Delete this font?": "هل تريد حذف هذا الخط؟",
  "Back to relay": "العودة إلى المرحل",
  Documentation: "التوثيق",
  "Self-host": "استضافة ذاتية",
  "Run your own Harbor Relay": "قم بتشغيل Harbor Relay الخاص بك",
  "Two paths: Harbor handles the deploy for you, or you do it yourself with wrangler.":
    "مساران: يتولى Harbor عملية النشر نيابةً عنك، أو تقوم بها بنفسك باستخدام wrangler.",
  Requirements: "المتطلبات",
  "A free Cloudflare account.": "حساب Cloudflare مجاني.",
  "About two minutes for the auto-deploy path.": "حوالي دقيقتين لمسار النشر التلقائي.",
  "Auto-deploy from Harbor": "النشر التلقائي من Harbor",
  "Open Settings, then Harbor Relay.": "افتح الإعدادات، ثم Harbor Relay.",
  "Pick the Cloudflare account to deploy under.": "اختر حساب Cloudflare للنشر ضمنه.",
  "Manual deploy with wrangler": "النشر اليدوي باستخدام wrangler",
  "For users who want to deploy themselves or already have a wrangler workflow.":
    "للمستخدمين الذين يفضلون النشر بأنفسهم أو لديهم سير عمل باستخدام wrangler مسبقاً.",
  "Install wrangler and authenticate:": "قم بتثبيت wrangler وتسجيل الدخول:",
  "Deploy:": "قم بالنشر:",
  "Verify it works": "التحقق من أنه يعمل",
  "Sharing your relay": "مشاركة المرحل الخاص بك",
  Costs: "التكاليف",
  "Cloudflare Workers free tier:": "الباقة المجانية من Cloudflare Workers:",
  "100,000 requests per day.": "100,000 طلب في اليوم.",
  "10ms CPU time per request.": "10 مللي ثانية (ms) من وقت المعالج لكل طلب.",
  "Unlimited Durable Object storage at $0.20 per million reads.":
    "مساحة تخزين Durable Object غير محدودة بتكلفة 0.20$ لكل مليون قراءة.",
  Troubleshooting: "استكشاف الأخطاء وإصلاحها",
  Symptom: "المشكلة",
  Cause: "السبب",
  Fix: "الحل",
  "Health check returns 5xx": "اختبار الصحة يرجع رمز 5xx",
  "Worker crashed or hit memory limits": "انهيار الـ worker أو وصوله للحد الأقصى للذاكرة",
  "Check logs in Cloudflare dashboard, then redeploy":
    "تحقق من السجلات في لوحة تحكم Cloudflare، ثم أعد النشر",
  "Connection refused / DNS does not resolve": "تم رفض الاتصال / خادم DNS لا يمكنه الترجمة",
  "Worker deleted or URL wrong": "تم حذف الـ worker أو الرابط غير صحيح",
  "Re-run deploy or paste the correct URL": "أعد عملية النشر أو الصق الرابط الصحيح",
  "Watch Together rooms drop after 6 hours": "سقوط غرف المشاهدة معاً بعد 6 ساعات",
  "Durable Object idle eviction": "إخلاء حالة الخمول في Durable Object",
  "Expected. Rooms recreate on next join.":
    "متوقع. يتم إعادة إنشاء الغرف عند الانضمام في المرة القادمة.",
  "What the worker does": "ما الذي يفعله الـ worker",
  Password: "كلمة المرور",
  Spanish: "الإسبانية",
  "Spanish (Latin America)": "الإسبانية (أمريكا اللاتينية)",
  French: "الفرنسية",
  German: "الألمانية",
  Italian: "الإيطالية",
  Portuguese: "البرتغالية",
  Russian: "الروسية",
  Japanese: "اليابانية",
  Korean: "الكورية",
  Chinese: "الصينية",
  Hindi: "الهندية",
  Arabic: "العربية",
  Turkish: "التركية",
  Dutch: "الهولندية",
  Polish: "البولندية",
  Ukrainian: "الأوكرانية",
  Czech: "التشيكية",
  Hungarian: "المجرية",
  Romanian: "الرومانية",
  Swedish: "السويدية",
  Norwegian: "النرويجية",
  Danish: "الدنماركية",
  Finnish: "الفنلندية",
  Hebrew: "العبرية",
  Thai: "التايلندية",
  Vietnamese: "الفيتنامية",
  "Login to Stremio": "تسجيل الدخول إلى Stremio",
  "Brings in your library, watchlist, and installed addons.":
    "يجلب مكتبتك وقائمة المشاهدة والإضافات المثبتة.",
  "Remember me": "تذكرني",
  "Stays signed in on this device only.": "يظل مسجل الدخول على هذا الجهاز فقط.",
  "Signing in...": "جاري تسجيل الدخول...",
  "Create account": "إنشاء حساب",
  "Layout editor": "محرر التخطيط",
  "Click any control to edit it.": "انقر على أي عنصر تحكم لتعديله.",
  "Save as new profile...": "حفظ كملف شخصي جديد...",
  "Export as file": "تصدير كملف",
  "Import from file...": "استيراد من ملف...",
  "Reset this profile to factory defaults? Your tweaks on it will be lost.":
    "هل تريد إعادة ضبط هذا الملف الشخصي للإعدادات الافتراضية؟ ستفقد التعديلات التي قمت بها.",
  PREVIEW: "معاينة",
  Slot: "الخانة",
  Order: "الترتيب",
  Icon: "أيقونة",
  Visible: "مرئي",
  "Preview state": "حالة المعاينة",
  "Move to previous slot": "نقل للخانة السابقة",
  "Move to next slot": "نقل للخانة التالية",
  "Show this control": "إظهار عنصر التحكم",
  "Hide this control": "إخفاء عنصر التحكم",
  "Show this panel": "إظهار هذه اللوحة",
  "Hide this panel": "إخفاء هذه اللوحة",
  Deselect: "إلغاء التحديد",
  "Top · right": "أعلى · يمين",
  "Top · left": "أعلى · يسار",
  "Bottom · right": "أسفل · يمين",
  "Bottom · left": "أسفل · يسار",
  "Bottom · center": "أسفل · وسط",
  "Above bar · left": "فوق الشريط · يسار",
  "Left edge": "الحافة اليسرى",
  "Right edge": "الحافة اليمنى",
  Side: "الجانب",
  Corner: "الزاوية",
  Full: "كامل",
  Transport: "تحكم التشغيل",
  Info: "معلومات",
  Actions: "إجراءات",
  Playing: "قيد التشغيل",
  Paused: "مُوقف",
  Windowed: "شاشة عادية",
  Inactive: "غير نشط",
  Idle: "خامل",
  Recording: "تسجيل",
  Complete: "مكتمل",
  Error: "خطأ",
  "Title & info": "العنوان والمعلومات",
  "Time elapsed": "الوقت المنقضي",
  "Time remaining or duration": "الوقت المتبقي أو المدة",
  "DVR record (Live TV)": "تسجيل البث المباشر (DVR)",
  "Play / Pause": "تشغيل / إيقاف مؤقت",
  "Switch stream / TV Guide": "تغيير البث / دليل التلفزيون",
  "Picture-in-picture": "صورة داخل صورة",
  "Viewer avatars": "صور المشاهدين",
  Chat: "الدردشة",
  "Next-up episodes tab": "علامة تبويب الحلقات القادمة",
  "Series tab": "علامة تبويب السلسلة",
  "update.ready": "التحديث جاهز للتثبيت",
  "update.installing": "جارٍ تثبيت التحديث",
  "update.downloading": "جارٍ تنزيل التحديث",
  "update.failed": "فشل التحديث",
  "update.available": "تحديث متاح",
  "update.harborVersion": "Harbor {version}",
  "update.downloadComplete": "اكتمل التنزيل",
  "update.fetching": "جلب أحدث إصدار",
  "update.errorServer": "حدث خطأ أثناء الاتصال بخادم التحديث.",
  "update.later": "لاحقاً",
  "update.download": "تنزيل",
  "update.installRestart": "تثبيت وإعادة تشغيل",
  "update.restartAuto": "سيعاد تشغيل Harbor تلقائياً.",
  "update.tryAgain": "المحاولة مرة أخرى",
  "update.keepUsing": "استمر في استخدام Harbor أثناء التنزيل",
  "update.of": "{downloaded} من {total}",
  "Hide password": "إخفاء كلمة المرور",
  "Show password": "عرض كلمة المرور",
  "Watch Time": "وقت المشاهدة",
  Episodes: "الحلقات",
  Movie: "فيلم",
  "{n} episode": "{n} حلقة",
  "{n} episodes": "{n} حلقات",
  "Season {n}": "موسم {n}",
  "Play Together": "المشاهدة معاً",
  "Resume S{s}:E{e}": "متابعة م{s}:ح{e}",
  "No episodes available for this season.": "لا توجد حلقات متاحة لهذا الموسم.",
  "Top Picks for You": "أفضل اللقطات لك",
  "S{s} E{e}": "م{s} ح{e}",
  "Morning Lineup": "تشكيلة الصباح",
  "Easing into series": "بداية هادئة مع المسلسلات",
  "Slow-burn worlds and bright chapters worth opening with coffee.":
    "عوالم هادئة وفصول مشرقة تستحق البدء بها مع القهوة.",
  "Good Morning": "صباح الخير",
  "Today's openers": "بدايات اليوم",
  "Series to ease into while the day is still quiet.": "مسلسلات هادئة بينما لا يزال اليوم هادئاً.",
  Daybreak: "الفجر",
  "First-light picks": "مختارات الضوء الأول",
  "Worlds to step into before the inbox catches up.": "عوالم تغوص فيها قبل أن تبدأ بالعمل.",
  "AM Picks": "مختارات الصباح",
  "Coffee-and-couch": "قهوة وأريكة",
  "Half-hours, anthologies, and a few epics for the morning routine.":
    "حلقات قصيرة ومسلسلات ملحمية للروتين الصباحي.",
  "Open the Day": "افتح اليوم",
  "Series with mileage": "مسلسلات طويلة",
  "Long-running comforts and new chapters worth pressing play on.":
    "مسلسلات مريحة طويلة وفصول جديدة تستحق المشاهدة.",
  "Quiet Hours": "ساعات هادئة",
  "Slow-burn starts": "بدايات هادئة",
  "Stories that reward your attention before the day gets loud.":
    "قصص تكافئ انتباهك قبل أن يصبح اليوم صاخباً.",
  "This Morning": "هذا الصباح",
  "Worth catching up on": "يستحق المتابعة",
  "What everyone has been quietly binging this week.": "ما يتابعه الجميع بهدوء هذا الأسبوع.",
  "Afternoon Picks": "مختارات الظهيرة",
  "Daytime watching": "مشاهدة نهارية",
  "Easy half-hours and lighter dramas to ride out the afternoon.":
    "حلقات خفيفة ودراما مريحة لقضاء فترة الظهيرة.",
  "Midday Lineup": "تشكيلة منتصف اليوم",
  "Between meetings": "بين الاجتماعات",
  "Episodes you can drop into without losing the thread.":
    "حلقات يمكنك مشاهدتها دون أن تفقد الحبكة.",
  "Afternoon Roll": "جولة الظهيرة",
  "Pick up an episode": "شاهد حلقة",
  "Lunch-break comedies and slow-cooker dramas, ready when you are.":
    "كوميديا استراحة الغداء ودراما هادئة، جاهزة متى ما أردت.",
  "The Long Lunch": "الغداء الطويل",
  "Series to disappear into": "مسلسلات لتغوص فيها",
  "Worlds wide enough for an hour or a whole free afternoon.":
    "عوالم واسعة تكفي لساعة أو لفترة ظهيرة كاملة.",
  "Daylight Watching": "مشاهدة في ضوء النهار",
  "Bright-side series": "مسلسلات الجانب المشرق",
  "Sharp comedies, sunny worlds, and the occasional binge bait.":
    "كوميديا حادة، عوالم مشمسة، ومسلسلات تجذبك للمشاهدة المتواصلة.",
  "Holdover Picks": "مختارات الاستراحة",
  "Carry it through the day": "اكملها خلال اليوم",
  "Companion series for whatever the afternoon throws at you.":
    "مسلسلات مرافقة لكل ما تواجهه في فترة الظهيرة.",
  "PM Picks": "مختارات المساء",
  "Couch hours": "ساعات الأريكة",
  "Series for the part of the day that runs on coffee and snacks.":
    "مسلسلات لجزء من اليوم يعتمد على القهوة والوجبات الخفيفة.",
  Tonight: "الليلة",
  "Tonight's lineup": "تشكيلة الليلة",
  "Prestige drama, weekly chapters, and series worth disappearing into.":
    "دراما راقية، حلقات أسبوعية، ومسلسلات تستحق الغوص فيها.",
  "Prime Time": "وقت الذروة",
  "What to watch tonight": "ماذا تشاهد الليلة",
  "Crowd-pleasers, prestige picks, and the kind of series people text about.":
    "مسلسلات محبوبة، مختارات راقية، ومسلسلات يتحدث عنها الناس.",
  Sundown: "الغروب",
  "Evening on the couch": "أمسية على الأريكة",
  "Drop-in chapters and long arcs for the post-dinner stretch.":
    "حلقات متفرقة وقصص طويلة لفترة ما بعد العشاء.",
  "Press Play": "اضغط تشغيل",
  "Tonight's marquee": "أبرز عروض الليلة",
  "The series that make the rest of the night disappear.":
    "المسلسلات التي تجعل بقية الليل يمر بلمح البصر.",
  "Tonight's Slate": "قائمة الليلة",
  "Episodes worth the evening": "حلقات تستحق السهرة",
  "What's hot this week, what's prestige forever, what's worth the hours.":
    "الأكثر رواجاً هذا الأسبوع، المسلسلات الخالدة، وما يستحق وقتك.",
  Showtime: "وقت العرض",
  "Tonight's main event": "الحدث الرئيسي الليلة",
  "Series for the part of the day you actually look forward to.":
    "مسلسلات للجزء الذي تنتظره حقاً من اليوم.",
  "Saved for Now": "محفوظة للآن",
  "Tonight's binge bait": "مسلسلات الليلة المتواصلة",
  "Pilots that pull you in and finales that earn the season.":
    "حلقات أولى تجذبك ونهايات تتوج الموسم.",
  "Late Night": "وقت متأخر من الليل",
  "After-hours picks": "مختارات ما بعد منتصف الليل",
  "Dark, immersive, and binge-worthy when the house is quiet.":
    "مظلمة، غامرة، وتستحق المشاهدة عندما يكون المنزل هادئاً.",
  "Past Midnight": "بعد منتصف الليل",
  "One more episode": "حلقة أخرى",
  "Series for the part of the night that won't let you sleep.":
    "مسلسلات للجزء من الليل الذي لن يدعك تنام.",
  "Witching Hour": "ساعة السحر",
  "Late-night chapters": "فصول آخر الليل",
  "Pull-you-under stories for the quietest part of the day.": "قصص تجذبك لأهدأ جزء من اليوم.",
  "Lights Out": "إطفاء الأنوار",
  "Headphone series": "مسلسلات بسماعات الرأس",
  "Slow, strange, and absorbing. Best with the lights down low.":
    "بطيئة، غريبة، وممتصة. أفضل مع إضاءة منخفضة.",
  "Insomnia Lineup": "تشكيلة الأرق",
  "Worth the lost hour": "تستحق الساعة الضائعة",
  "Dense plots and rich worlds for when sleep is not happening.":
    "حبكات مكثفة وعوالم غنية عندما لا يأتي النوم.",
  "Late Show": "عرض متأخر",
  "After the news": "بعد الأخبار",
  "Quiet dramas, sharp thrillers, and series you save for yourself.":
    "دراما هادئة، إثارة حادة، ومسلسلات تحفظها لنفسك.",
  "Night Owl": "بومة الليل",
  "While the world's asleep": "بينما العالم نائم",
  "Series with the patience to match your late-night hours.":
    "مسلسلات تتناسب مع ساعاتك المتأخرة من الليل.",
  "Trending This Week": "الشائع هذا الأسبوع",
  "On Tonight": "يعرض الليلة",
  "Premiered This Month": "عرض هذا الشهر",
  "From HBO": "من HBO",
  "Netflix Originals": "أعمال نتفليكس الأصلية",
  "Apple TV+": "أبل تي في+",
  AMC: "AMC",
  FX: "FX",
  "Disney+ Originals": "أعمال ديزني+ الأصلية",
  "Prime Video": "برايم فيديو",
  "Limited Series & Miniseries": "مسلسلات محدودة وقصيرة",
  "Prestige Drama": "دراما راقية",
  "Comedy Series": "مسلسلات كوميدية",
  "Crime & Mystery": "جريمة وغموض",
  "Sci-Fi & Fantasy": "خيال علمي وفانتازيا",
  "Documentary Series": "مسلسلات وثائقية",
  "All-Time Great Series": "أعظم المسلسلات على الإطلاق",
  "Iconic Long-Runners": "مسلسلات أيقونية طويلة",
  "K-Drama": "دراما كورية",
  "British Television": "تلفزيون بريطاني",
  "Top 10 Series Today": "أفضل 10 مسلسلات اليوم",
  "Top Series": "أفضل المسلسلات",
  "Top Drama": "أفضل مسلسلات الدراما",
  "Top Comedy": "أفضل مسلسلات الكوميديا",
  "Top Crime": "أفضل مسلسلات الجريمة",
  "Top Sci-Fi": "أفضل مسلسلات الخيال العلمي",
  "Top Thriller": "أفضل مسلسلات الإثارة",
  "Top Mystery": "أفضل مسلسلات الغموض",
  "Top Action": "أفضل مسلسلات الأكشن",
  "Top Animation": "أفضل مسلسلات الرسوم المتحركة",
  "Top Adventure": "أفضل مسلسلات المغامرة",
  "Top Fantasy": "أفضل مسلسلات الفانتازيا",
  "Top Documentary": "أفضل المسلسلات الوثائقية",
  "Top Romance": "أفضل مسلسلات الرومانسية",
  "Top Horror": "أفضل مسلسلات الرعب",
  "Pick up where you left off": "أكمل من حيث توقفت",
  Discover: "اكتشف",
  Browse: "تصفّح",
  "Top rising": "الأكثر صعودًا",
  "Most starred in 24 hours": "الأكثر نجومًا خلال 24 ساعة",
  Adult: "للبالغين",
  Hide: "إخفاء",
  All: "الكل",
  "Loading the catalog": "جاري تحميل الفهرس",
  "Addon order synced to your Stremio account": "رُتِّبت الإضافات وزُامنت مع حساب Stremio",
  "Addon order saved on this device": "رُتِّبت الإضافات وحُفظت على هذا الجهاز",
  "Curated for popularity and reliability. No paid placements. Install anything else by URL on the Browse tab.":
    "محتوى منتقى للشعبية والموثوقية. لا توجد مواضع مدفوعة. ثبّت أي شيء آخر عبر URL في تبويب التصفّح.",
  "Sign in to sync your addons across devices": "سجّل الدخول لمزامنة إضافاتك عبر الأجهزة",
  "Anything you install in Harbor pushes back to your Stremio account so it shows up on mobile too. Sign in via the avatar in the bottom-left of the sidebar.":
    "كل ما تثبّته في Harbor يُرسَل إلى حساب Stremio الخاص بك ليظهر على الجوال أيضًا. سجّل الدخول عبر الصورة الرمزية في أسفل يسار الشريط الجانبي.",
  Starters: "البدايات",
  "Common picks for a fresh setup.": "اختيارات شائعة لإعداد جديد.",
  Streaming: "البث",
  "Where your video comes from": "مصدر مقاطع الفيديو الخاصة بك",
  Catalogs: "الفهارس",
  "Posters, ratings, lists": "الملصقات والتقييمات والقوائم",
  Subtitles: "الترجمات",
  "Captions in your language": "تسميات توضيحية بلغتك",
  "Kitsu, MAL, season-aware": "Kitsu و MAL ومدرك للمواسم",
  Torrents: "التورنت",
  "P2P sources, debrid-ready": "مصادر P2P، جاهز للـ debrid",
  "Live TV": "بث مباشر",
  "OTA channels + IPTV": "قنوات OTA + IPTV",
  New: "جديد",
  "You've reached the end · {n} addons": "لقد وصلت إلى النهاية · {n} إضافة",
  "Trending tracks star growth across your Harbor visits. Open the addons page again tomorrow and the top risers will appear here.":
    "يتتبّع قسم الرائج نمو النجوم عبر زيارات Harbor. افتح صفحة الإضافات غدًا وستظهر هنا أكثر الإضافات صعودًا.",
  "Head to Discover. Cinemeta and OpenSubtitles cover the basics; Torrentio + a debrid key cover almost everything else.":
    "انتقل إلى الاكتشاف. يغطّي Cinemeta و OpenSubtitles الأساسيات؛ ويغطّي Torrentio + مفتاح debrid كل شيء تقريبًا.",
  "No installed addon matches that.": "لا توجد إضافة مثبّتة تطابق ذلك.",
  "Clear the search to see all {n} installed.": "امسح البحث لرؤية جميع الـ{n} مثبّتة.",
  Reorder: "إعادة الترتيب",
  "Change the order addons are tried in": "غيّر الترتيب الذي تُجرَّب به الإضافات",
  "Off · catalogs and streams hidden": "مغلق · الفهارس والبث مخفيّة",
  Enabled: "مفعّل",
  Disabled: "معطّل",
  "Click to turn off": "انقر للإيقاف",
  "Click to turn on": "انقر للتشغيل",
  "Turn off": "إيقاف",
  "Turn on": "تشغيل",
  "Re-configure this addon and apply the updated link": "أعد تهيئة هذه الإضافة وطبّق الرابط المُحدَّث",
  Manage: "إدارة",
  Uninstalling: "جارٍ الإزالة",
  Official: "رسمي",
  Community: "مجتمعي",
  Addon: "إضافة",
  Rising: "صاعد",
  star: "نجمة",
  stars: "نجوم",
  "in 24h": "خلال 24 ساعة",
  Removing: "جارٍ الإزالة",
  Remove: "إزالة",
  "Copy URL": "نسخ الرابط",
  "Opening stremio-addons.net in your browser to sign in and rate":
    "يتم فتح stremio-addons.net في متصفّحك لتسجيل الدخول والتقييم",
  "Stremio link": "رابط Stremio",
  copied: "تم النسخ",
  "Couldn't copy. Select the URL manually.": "تعذّر النسخ. حدّد الرابط يدويًا.",
  "Hide the full URL": "إخفاء الرابط الكامل",
  "URLs can carry debrid keys or tokens; reveal when you need to copy":
    "قد تحتوي الروابط على مفاتيح debrid أو رموز؛ اكشفها عند الحاجة للنسخ",
  Copy: "نسخ",
  "Hidden by default. Manifest paths often carry API keys (debrid tokens, OMDB keys, etc.) you don't want over a shoulder.":
    "مخفي افتراضيًا. كثيرًا ما تحمل مسارات المانيفست مفاتيح API (رموز debrid، مفاتيح OMDB، إلخ) لا تريد أن يراها أحد.",
  "Version and capabilities come straight from the addon's manifest. Ratings and categories come from the":
    "تأتي الإصدار والقدرات مباشرة من مانيفست الإضافة. تأتي التقييمات والفئات من",
  "community API. Star, browse, and contribute on their site.":
    "واجهة برمجة المجتمع. قيّم وتصفّح وساهم على موقعهم.",
  "More like this": "المزيد من هذا النوع",
  "Recommended for you": "موصى لك",
  Cancel: "إلغاء",
  "'s setup page. Pick your options, then copy the install link it gives you and paste it below to update the addon.":
    " صفحة الإعداد. اختر خياراتك، ثم انسخ رابط التثبيت الذي يمنحك إياه والصقه أدناه لتحديث الإضافة.",
  "'s setup page in Harbor's built-in browser. Pick your options. When you click Install on their page, Harbor catches the link automatically and updates the addon.":
    " صفحة الإعداد في متصفّح Harbor المدمج. اختر خياراتك. عند النقر على تثبيت في صفحتهم، يلتقط Harbor الرابط تلقائيًا ويحدّث الإضافة.",
  "Saving to library": "الحفظ في المكتبة",
  "Syncing to Stremio": "المزامنة مع Stremio",
  Done: "تم",
  "We need to check your age before you sail ahead. Three quick questions a working adult would know in their sleep. Get them all right and the adult shelf opens.":
    "نحتاج للتحقق من عمرك قبل الإبحار. ثلاثة أسئلة سريعة يعرفها أي شخص بالغ يعمل حتى في نومه. أجب عنها جميعًا بشكل صحيح وسيفتح قسم المحتوى الراشد.",
  "Top 10": "أفضل 10",
  "Add a TMDB key in Settings to load Arabic content.":
    "أضف مفتاح TMDB في الإعدادات لتحميل المحتوى العربي.",

  "Add a TMDB key for the full Harbor": "أضف مفتاح TMDB لتجربة Harbor الكاملة",
  "Free key unlocks Trending, In Theaters, and per-service catalogs. 60 seconds.":
    "المفتاح المجاني يفتح الرائج وفي دور السينما وكتالوجات كل خدمة. 60 ثانية.",

  "Built for desktop resolutions": "مصمَّم لدقّات شاشات سطح المكتب",
  "This instance of Harbor is made for desktop. Our standalone iOS and Android apps are coming soon, each with a bespoke, mobile-first experience built for its native platform.":
    "هذه النسخة من Harbor مصمَّمة لسطح المكتب. تطبيقاتنا المستقلّة لنظامي iOS و Android قادمة قريبًا، لكلٍّ منها تجربة مخصّصة تركّز على الهاتف أولًا ومبنيّة لمنصّته الأصلية.",
  "For now, please open this site on a desktop, or build Harbor from source.":
    "في الوقت الحالي، يُرجى فتح هذا الموقع على سطح المكتب، أو بناء Harbor من المصدر.",
  "Build from source": "البناء من المصدر",

  "Wrong PIN": "رمز PIN خاطئ",
  "PINs didn't match. Start over.": "رمزا PIN غير متطابقين. ابدأ من جديد.",
  "Enter your PIN": "أدخل رمز PIN الخاص بك",
  "Set a 4-digit PIN": "عيّن رمز PIN مكوّنًا من 4 أرقام",
  "Confirm your PIN": "أكّد رمز PIN الخاص بك",
  "Parental controls are on. Enter your PIN to access settings.":
    "الرقابة الأبوية مفعّلة. أدخل رمز PIN للوصول إلى الإعدادات.",
  "You'll need this to access settings while controls are on.":
    "ستحتاج إلى هذا للوصول إلى الإعدادات أثناء تفعيل الرقابة.",
  "Type the same PIN one more time.": "اكتب رمز PIN نفسه مرة أخرى.",
  "Focus PIN entry": "تركيز إدخال PIN",
  PIN: "رمز PIN",
  "Type on your keyboard or tap the digits above.":
    "اكتب باستخدام لوحة المفاتيح أو انقر الأرقام أعلاه.",

  "Top 100 Actors": "أفضل 100 ممثل",
  "Most popular performers right now": "أكثر الفنانين شهرة الآن",
  "Top 100 Directors": "أفضل 100 مخرج",
  "Filmmakers leading the conversation": "صنّاع أفلام يتصدّرون المشهد",
  "Top 100 Producers": "أفضل 100 منتج",
  "Names behind the biggest productions": "أسماء وراء أضخم الإنتاجات",
  "Top 100 Writers": "أفضل 100 كاتب",
  "Pens currently in demand": "أقلام مطلوبة حاليًا",
  "{subtitle} · ranked by current popularity": "{subtitle} · مرتّبة حسب الشهرة الحالية",
  "Filter by name or title": "التصفية حسب الاسم أو العنوان",
  "Loading…": "جارٍ التحميل…",
  "No matches.": "لا توجد نتائج.",
  "Back to top": "العودة إلى الأعلى",
  "Best known for": "اشتهر بـ",
  "No credits available": "لا توجد أعمال متاحة",
  "View profile": "عرض الملف الشخصي",
  "Open on IMDb": "فتح على IMDb",
  "Open {name}": "فتح {name}",

  "Skip setup": "تخطّي الإعداد",
  "Skip for now": "تخطّي الآن",
  "Get Started": "ابدأ الآن",
  "Enter Harbor": "ادخل إلى Harbor",

  "A client for the Stremio protocol. Two minutes to set up; most of it optional. You stay in control of every key.":
    "عميل لبروتوكول Stremio. دقيقتان للإعداد، ومعظمه اختياري. تبقى أنت المتحكّم في كل مفتاح.",
  Current: "الحالي",
  Yours: "ما يخصّك",
  Quiet: "هادئ",
  "Trending, in theaters, what's on every streamer.":
    "الرائج، وما يُعرض في السينما، وما هو متاح على كل منصّة بث.",
  "Your Stremio library + addons sync in untouched.":
    "تتزامن مكتبة Stremio وإضافاتك كما هي دون تغيير.",
  "No telemetry, no servers, no bundled keys.": "لا قياس عن بُعد، ولا خوادم، ولا مفاتيح مُضمَّنة.",

  "For watching things": "لمشاهدة الأشياء",

  "Harbor curated": "اختيار Harbor",
  "Hero, Top 10, Trending, In Theaters, per-service rails. Your addons append underneath.":
    "الواجهة البارزة، وأفضل 10، والرائج، وفي دور السينما، وصفوف كل خدمة. تُضاف إضافاتك في الأسفل.",
  "Classic Stremio": "Stremio الكلاسيكي",
  "Continue Watching, then your addon catalogs in install order. No hero, no Harbor rails.":
    "متابعة المشاهدة، ثم كتالوجات إضافاتك بترتيب التثبيت. بلا واجهة بارزة، وبلا صفوف Harbor.",
  "Pick a home layout": "اختر تخطيط الصفحة الرئيسية",
  "You can switch later in Settings under Library & metadata.":
    "يمكنك التبديل لاحقًا من الإعدادات ضمن المكتبة والبيانات الوصفية.",

  "Step 1 · Metadata": "الخطوة 1 · البيانات الوصفية",
  "Free, two-minute signup. Unlocks Trending, In Theaters Now, Top Rated, and per-streaming catalogs (Netflix, Disney+, Hulu, …). Your key stays on this machine.":
    "تسجيل مجاني في دقيقتين. يفتح الرائج، وفي دور السينما الآن، والأعلى تقييمًا، وكتالوجات كل خدمة بث (Netflix و Disney+ و Hulu، …). يبقى مفتاحك على هذا الجهاز.",
  "Get a free key at themoviedb.org": "احصل على مفتاح مجاني من themoviedb.org",
  "v3 API key": "مفتاح API الإصدار 3",
  Verify: "تحقّق",
  "Skip if you'd rather just use Cinemeta. Harbor still works, you'll just see fewer rails.":
    "تخطَّ إن كنت تفضّل استخدام Cinemeta فقط. سيظل Harbor يعمل، لكنك سترى صفوفًا أقل.",

  "Sign-in failed": "فشل تسجيل الدخول",
  "Step 2 · Stremio": "الخطوة 2 · Stremio",
  "You're in": "تم تسجيل دخولك",
  "Library and addons will sync in once you're past setup.":
    "ستتزامن المكتبة والإضافات بمجرد انتهائك من الإعداد.",
  "Signing in…": "جارٍ تسجيل الدخول…",
  "Sign in to Stremio": "تسجيل الدخول إلى Stremio",
  "Don't have an account?": "ليس لديك حساب؟",
  "Create one": "أنشئ واحدًا",
  "Bring in your library": "أحضِر مكتبتك",
  "Sign in to mirror your Continue Watching, watchlist, and any addons you've already curated. Optional; Harbor works fully signed-out.":
    "سجّل الدخول لتعكس متابعة المشاهدة وقائمة مشاهدتك وأي إضافات نظّمتها مسبقًا. اختياري؛ يعمل Harbor بالكامل دون تسجيل الدخول.",
  "Sign in with": "تسجيل الدخول عبر",
  "Don't have an account? Create one →": "ليس لديك حساب؟ أنشئ واحدًا ←",

  "Step 3 · Streaming": "الخطوة 3 · البث",
  "Pick what you actually use": "اختر ما تستخدمه فعلًا",
  "Harbor pulls the most popular titles each service has right now. Toggle off anything you don't subscribe to.":
    "يجلب Harbor أكثر العناوين شهرة لدى كل خدمة الآن. عطّل أي خدمة لست مشتركًا بها.",
  "These rails activate once a TMDB key is set. You can come back to this anytime in Settings.":
    "تُفعَّل هذه الصفوف بمجرد تعيين مفتاح TMDB. يمكنك العودة إلى هذا في أي وقت من الإعدادات.",

  "Step 4 · Subtitles": "الخطوة 4 · الترجمات",
  "Pick your subtitle languages": "اختر لغات الترجمة",
  "When playback starts, Harbor finds and loads a subtitle in one of these languages automatically. The first available match wins, so put your main language first.":
    "عند بدء التشغيل، يبحث Harbor تلقائيًا عن ترجمة بإحدى هذه اللغات ويحمّلها. تُعتمد أول مطابقة متاحة، لذا ضع لغتك الأساسية أولًا.",

  "You're set.": "أنت جاهز.",
  "TMDB connected. {n} streaming {services} on. Welcome aboard.":
    "تم ربط TMDB. {n} {services} بث مفعّلة. مرحبًا بك.",
  "Running on Cinemeta for now. Add a TMDB key from Settings whenever you're ready.":
    "يعمل على Cinemeta حاليًا. أضف مفتاح TMDB من الإعدادات متى كنت مستعدًا.",
  service: "خدمة",
  services: "خدمات",

  "Your Discovery Queue": "طابور الاستكشاف الخاص بك",
  "{count} picks ready": "{count} اختيارات جاهزة",
  "Explore your queue": "استكشف طابورك",
  "{count} films": "{count} فيلم",
  Collection: "مجموعة",
  "Browse by Award": "تصفح حسب الجائزة",
  "Academy Awards": "جوائز الأوسكار",
  "Best Picture and beyond": "أفضل فيلم وما بعده",
  "Golden Globes": "جولدن جلوب",
  "Film and television": "السينما والتلفزيون",
  BAFTA: "بافتا",
  "The British Academy": "الأكاديمية البريطانية",
  Emmys: "إيمي",
  "Television's finest": "أفضل الأعمال التلفزيونية",
  "SAG Awards": "جوائز نقابة ممثلي الشاشة",
  "Chosen by actors": "باختيار الممثلين",
  "Critics' Choice": "اختيار النقاد",
  "The critics' cut": "حسب اختيار النقاد",
  Cannes: "مهرجان كان",
  "Palme d'Or": "السعفة الذهبية",
  Venice: "مهرجان البندقية",
  "Golden Lion": "الأسد الذهبي",
  Berlinale: "مهرجان برلين",
  "Golden Bear": "الدب الذهبي",
  "Browse by Language": "تصفح حسب اللغة",
  Action: "أكشن",
  Adventure: "مغامرة",
  Drama: "دراما",
  Romance: "رومانسي",
  Mystery: "غموض",
  "Sci-Fi": "خيال علمي",
  Fantasy: "فانتازيا",
  Horror: "رعب",
  Comedy: "كوميديا",
  Animation: "أنميشن",
  Music: "موسيقى",
  "Share collection": "مشاركة المجموعة",
  "Anyone with the link can open this collection once your Harbor server is live.":
    "يمكن لأي شخص لديه الرابط فتح هذه المجموعة بمجرد أن يصبح خادم Harbor الخاص بك نشطًا.",
  "Paste this code into Harbor to open the collection.": "الصق هذا الرمز في Harbor لفتح المجموعة.",
  "Shared to the community": "تمت المشاركة مع المجتمع",
  "Share to the community": "المشاركة مع المجتمع",
  "Listed collections will appear in community browse when that rolls out.":
    "ستظهر المجموعات المُدرَجة في تصفّح المجتمع عند إطلاق هذه الميزة.",
  "Sign in to get a shareable link.": "سجّل الدخول للحصول على رابط قابل للمشاركة.",
  Link: "الرابط",
  Code: "الرمز",
  "Removed from {page}": "تمت الإزالة من {page}",
  "That page is full": "هذه الصفحة ممتلئة",
  "Added to {page}": "تمت الإضافة إلى {page}",
  "Show as a row on": "العرض كصف في",
  "The collection shows up as its own row you can reorder or hide from that page.":
    "تظهر المجموعة كصف مستقل يمكنك إعادة ترتيبه أو إخفاؤه من تلك الصفحة.",
  "This collection is no longer here.": "لم تعد هذه المجموعة موجودة هنا.",
  "Back to collections": "العودة إلى المجموعات",
  "Add to a page": "إضافة إلى صفحة",
  "Open the editor to add the movies, shows, and manga that belong in this collection.":
    "افتح المحرّر لإضافة الأفلام والمسلسلات والمانغا التي تنتمي إلى هذه المجموعة.",
  "Add titles": "إضافة عناوين",
  Tags: "الوسوم",
  "Add up to {max} tags so people can find this in the community.":
    "أضِف حتى {max} وسوم حتى يتمكن الآخرون من العثور على هذا في المجتمع.",
  "Remove tag": "إزالة الوسم",
  "Tag limit reached": "تم بلوغ حدّ الوسوم",
  "Add a tag": "أضِف وسمًا",
  "Paused on Trakt": "متوقّف مؤقتًا على Trakt",
};

export default misc;
