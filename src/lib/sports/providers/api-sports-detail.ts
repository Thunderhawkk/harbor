...
function gridPlaces(starters: Raw[]): Map<string, number> {
  const parsed = starters.map((s, index) => {
    const [row, col] = String(s.player?.grid ?? "")
      .split(":")
      .map(Number);
    return {
      id: idOf(s.player?.id),
...
function statRows(homeBox: Raw | undefined, awayBox: Raw | undefined): MatchTeamStatRow[] {
  const list: Raw[] = [
    ...(Array.isArray(homeBox?.statistics) ? homeBox.statistics : []),
    ...(Array.isArray(awayBox?.statistics) ? awayBox.statistics : []),
  ];
...
function footballEvent(e: Raw, index: number, cameOn: Set<string>): MatchEvent {
  ...
  if (kind === "goal" && !lower.includes("missed")) type = "goal";
    ...
    type = lower.includes("red") || lower.includes("second") ? "red_card" : "yellow_card";
  ...
  if (type === "substitution" && cameOn.has(idOf(e.assist?.id)))
    [player, assist] = [assist, player];
