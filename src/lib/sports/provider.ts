...
async function pooled<T>(
  ...
  const out: T[][] = Array.from({ length: items.length }, () => []);
  let cursor = 0;
  const worker = async () => {
    for (;;) {
      const i = cursor++;
...
export function fetchGameSummary(game: SportsGame): Promise<SportsMatchDetail | null> {
  ...
  if (source === ESPN_ID) return fetchMatchSummary(game.league, game.id, game.startMs);
  ...
  const def = provider
    ...
    .find(
      (l) =>
        l.tag === game.league ||
        (source === "api-sports" && l.key === apiLeagueForHub(game.league)),
    );
  return def
    ? provider
        .fetchSummary(def.key, game.id)
        .then((detail) => (detail ? { ...detail, league: game.league } : null))
