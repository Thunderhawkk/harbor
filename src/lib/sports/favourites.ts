...
import { createTeamCatalog } from "./team-catalog";
import { readTeamCatalogDisk, writeTeamCatalogDisk } from "./team-catalog-storage";
import { HUB_LEAGUES, parseDbEvents } from "./hub-data";
import { SPORTSDB_LEAGUES } from "./providers/sportsdb";
import { loadTeamProviderJson, readSportsDbTeamSchedule } from "./team-provider";
import { setItemWithRecovery } from "@/lib/storage-recovery";
import { sortGames, type LeagueDef, type SportsGame, type SportsSide } from "@/lib/sports/espn";
...
export type SportsTeam = FavouriteTeam & { shortName: string; aliases?: string[] };
...
function normalise(raw: unknown): SportsFavourites {
  ...
  return { leagues, teams, home, personalized: obj.personalized === true };
}
...
export function leagueOf(leagueKey: string): LeagueDef | undefined {
  return (
    HUB_LEAGUES.find((l) => l.key === leagueKey) ??
    SPORTSDB_LEAGUES.find((l) => l.key === leagueKey)
  );
}
...
export function matchSides(
  game: SportsGame,
  team: TeamRef,
): { own: SportsSide; opponent: SportsSide; atHome: boolean } {
...
// Team selection can load without importing the Sports hub first.
allowDirectHost(BASE);
const teamCatalog = createTeamCatalog({
  request: async (url, signal) => {
    if (new URL(url).hostname === "www.thesportsdb.com") return loadTeamProviderJson(url, signal);
    const response = await safeFetch(url, { signal });
    if (!response.ok) throw new Error(`Team catalog HTTP ${response.status}`);
    return response.json();
  },
  read: () => localStorage.getItem("harbor.sports.team-catalog.v1"),
  readAsync: readTeamCatalogDisk,
  write: (json) => {
    void writeTeamCatalogDisk(json);
  },
});

export function cachedLeagueTeams(leagueKey: string): SportsTeam[] {
  return teamCatalog.cached(leagueKey);
}

export function teamCatalogStatus(leagueKey: string) {
  return teamCatalog.status(leagueKey);
}

export function teamCatalogIsPartial(leagueKey: string): boolean {
  return teamCatalog.isPartial(leagueKey);
}
...
export function fetchLeagueTeams(
  ...
  return def ? teamCatalog.fetch(def, options?.force) : Promise.resolve([]);
}
...
function parseScheduleEvents(events: unknown[], def: LeagueDef): SportsGame[] {
  ...
  for (const raw of events) {
    ...
    const comps = Array.isArray(ev.competitions)
      ? (ev.competitions as Record<string, unknown>[])
      : [];
    const comp = comps[0];
    if (!comp) continue;
    const cs = Array.isArray(comp.competitors)
    ...
    const type = ((comp.status as Record<string, unknown> | undefined)?.type ?? {}) as Record<
      string,
      unknown
    >;
...
async function fetchTeamScheduleRaw(def: LeagueDef, teamId: string): Promise<SportsGame[]> {
  if (/^\d+$/.test(def.path)) {
    const events = await readSportsDbTeamSchedule(def, teamId, loadTeamProviderJson);
    return parseDbEvents(events, def);
  }
...
export function fetchTeamSchedule(leagueKey: string, teamId: string): Promise<SportsGame[]> {
  ...
  return throughCache(
    ...
    key,
    SCHEDULE_TTL,
    () => fetchTeamScheduleRaw(def, teamId),
    [],
  );
}
